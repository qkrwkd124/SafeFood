	...
	$('document').ready(function () {
		...
		$('#addNaver').click(function(){
			...
			<%
					...
					if(!((Customer)session.getAttribute("c")).getId().equals("1")){
			%>
				alert('관리자만 글쓸 수 있습니다.')
				return;
				<%}%>
				
				$('#mainContent').load("addgongji.dd")
				$('#mainContent').css("height","800px")
			<%}%>
		})
		$(".hover").mouseleave(
...
<div id="total">
	<div id="title">
		<header class="goodheader">안전한 먹거리 프로젝트 공지사항</header>
		<hr id="titleBar1">
		<br>
		<br>
		<br>
		...
		<div id="mainContent">
			...
			<table class="table-hover">
				<tr style="font-size: 20px;">
					...
					<td width="200px" align="center">글쓴이</td>
					<td width="200px" align="center">날짜</td>
				</tr>
				<tr>
					<td width="800px" colspan="4" bgcolor="hotpink" height="3px"></td> 
				</tr>
				<template v-if="find=='title'">
		 			<template v-for="naver in info" v-if="naver.title.includes(content)">
					<tr  style="font-size: 17px;">
						<td align="center">{{naver.num }}</td>
						<td align="center"><a :href='"gongjidetail.dd?num="+naver.num+"&writer="+naver.writer'>{{naver.title }}</a></td>
